...
try:
    ...
    sys.stderr.write(
        "Error: The 'vmd-python' dependency could not be located. Please follow instructions "
...
def compute_frame_contacts(
    ...
    frame_contacts = []
    if "sb" in itypes:
        frame_contacts += compute_salt_bridges(
            molid,
            frame,
            index_to_atom,
            sele1,
    ...
    if "pc" in itypes:
        frame_contacts += compute_pi_cation(
            molid, frame, index_to_atom, sele1, sele2, geom_criteria
        )
    if "ps" in itypes:
        frame_contacts += compute_pi_stacking(
            molid,
            frame,
            index_to_atom,
            sele1,
    ...
    if "ts" in itypes:
        frame_contacts += compute_t_stacking(
            molid,
            frame,
            index_to_atom,
            sele1,
    ...
    if "vdw" in itypes:
        frame_contacts += compute_vanderwaals(
            molid, frame, index_to_atom, sele1, sele2, geom_criteria, disulfide_cys
        )
    if "hb" in itypes:
        frame_contacts += compute_hydrogen_bonds(
            molid,
            frame,
            index_to_atom,
            sele1,
    ...
    if "hp" in itypes:
        frame_contacts += compute_hydrophobics(
            molid, frame, index_to_atom, sele1, sele2, geom_criteria, disulfide_cys
        )
...
def compute_fragment_contacts(
    ...
    for frame_idx in range(num_frag_frames):
        ...
        fragment_contacts += compute_frame_contacts(
            molid,
            frame_idx,
            itypes,
            geom_criterion_values,
    ...
    print(
        "Finished computing contacts for fragment %d: %d frames from %d to %d in strides of %d taking %s s"
        % (
            frag_idx,
            num_frag_frames,
            beg_frame,
            beg_frame + num_frag_frames * stride - 1,
            stride,
...
def compute_contacts(
    ...
    inputqueue = Queue()
    print(
        "Processing %s (frame %d to %d with stride of %d) as %d fragments\n"
    ...
    if (
        num_workers == 1
    ):  # Run everything in series (in addition to being slow it will consume memory)
        ...
        workers = [
            Process(target=contact_worker, args=(inputqueue, resultsqueue))
            for _ in range(num_workers)
        ]
        for w in workers:
            w.start()
...
def contact_consumer(resultsqueue, output_fd, itypes, beg, end, stride, distout):
    ...
    output_fd.write(
        "# total_frames:%d beg:%d end:%d stride:%d interaction_types:%s\n"
        % (total_frames, beg, end, stride, ",".join(itypes))
    )

    if distout:
        output_fd.write(
            "# Columns: frame, interaction_type, atom_1, atom_2[, atom_3[, atom_4]], distance_1-2\n"
        )
    else:
        output_fd.write(
    ...
    while True:
        ...
        if result == "DONE":
            assert (
                not resultheap
            ), "resultheap not empty .. unwritten contacts on termination"
            output_fd.close()
            break
        ...
        while resultheap and waiting_for_fragment == resultheap[0][0]:
            ...
            for interaction in contacts:
                for a in range(
                    2, len(interaction) - 1 if distout else len(interaction)
                ):
